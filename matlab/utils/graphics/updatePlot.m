...
%   check, so it is safe to call even when a handle was never created.
    ...
    % ------------------------------------------------------------------
    % 7. Secondary trajectory (gtTrajectory)
    %    → Ground truth when EKF is on, unused otherwise
    %    Only drawn if the handle was created at setup (showGroundTruth = true)
    % ------------------------------------------------------------------
